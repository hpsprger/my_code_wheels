...
int sort_compare_key_func(HashEntry *a, HashEntry *b) 
{  
    if (a->key.data > b->key.data) return 1;
    if (a->key.data < b->key.data) return -1;
    return 0;
}
...
{ 
    ...
    while (1) {
        ...
        if ( order =='q') {
            ...
            if (pResult) {
                printf("found entry: key.data=%d value=0x%x...\n", pResult->key.data, pResult->value.data);
            } else {
                printf("findKey.a=%d not found...\n", findKey.data);
            }
        } else if (order == 'd') {   
            printf("delete entry....\n");
            data = 0;
            ...
            printf("key sort...\n");
            HASH_SORT(pHashTableEntryHead, sort_compare_key_func);
        } else if (order == 'x') {
            printf("value sort...\n");
            ...
            for (pTmp = pHashTableEntryHead; pTmp != NULL; pTmp=pTmp->hh.next) {
                printf("entry: key.data=%d value.data=%d \n", pTmp->key.data, pTmp->value.data);
            }
        }
        scanf("%c",&order);
    }
